...
import { version1LayoutConfig } from './';
...
const initalLayoutProps: IDemo1LayoutProviderProps = {
  layout: version1LayoutConfig, // Default layout configuration
...
const Version1LayoutProvider = ({ children }: PropsWithChildren) => {
  ...
  const getLayoutConfig = () => {
    return deepMerge(version1LayoutConfig, getLayout(version1LayoutConfig.name));
  };
  ...
  const setSidebarCollapse = (collapse: boolean) => {
    ...
    updateLayout(version1LayoutConfig.name, updatedLayout); // Updates the layout with the collapsed state
...
export { Version1LayoutProvider, useDemo1Layout };
